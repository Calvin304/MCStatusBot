...
module.exports = server => new Promise(resolve => {
    ...
    mc.ping({host: url.hostname, port: url.port || 25565, checkTimeoutInterval: 10*1000},(err, pingResults) => {
        ...
        .addField("There " + ((pingResults.players.online === 1)?"is **":"are **") + pingResults.players.online + "/" + pingResults.players.max +"** players online", ((pingResults.players.sample[0] == undefined)?"n/a":"`" + pingResults.players.sample.map(player => player.name).join(", ") + "`"), false)
