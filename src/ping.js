...
module.exports = server => new Promise(resolve => {
    ...
    mc.ping({host: url.hostname, port: url.port || 25565},(err, pingResults) => {
        ...
        .addField("There " + ((pingResults.players.online === 1)?"is **":"are **") + pingResults.players.online + "/" + pingResults.players.max +"** players online", (((pingResults.players.sample || []).length == 0)?"​":"`" + pingResults.players.sample.map(player => player.name).join(", ") + "`"), false)
